...
"""
统计db中不同类型键的具体数量以及它们在整个数据库中所占的百分比
"""
...
class DbSampler:
  ...
  def sample(self):
    # 键类型计数器 
    type_counter = {"string": 0, "list": 0, "hash": 0, "set": 0, "zset": 0, "stream": 0, }
    # 遍历整个数据库 
    for key in self.client.scan_iter():
      # 获取键的类型 
      type = self.client.type(key)  # 对相应的类型计数器执行加1操作 
      type_counter[type] += 1

    # 获取数据库的大小
    db_size = self.client.dbsize()  # 打印结果 
